...
class ResDown(MultiStageFeature):
    def __init__(self, pretrain=False):
        ...
        if pretrain:
            load_pretrain(self.features, 'pretrained/resnet.model')
...
class Custom(SiamMask):
    ...
    def track(self, examplar_feature, search):
        search = self.features(search)
        rpn_pred_cls, rpn_pred_loc = self.rpn(examplar_feature, search)
        return rpn_pred_cls, rpn_pred_loc

    def track_mask(self, examplar_feature, search):
        self.feature, self.search = self.features.forward_all(search)
        rpn_pred_cls, rpn_pred_loc = self.rpn(examplar_feature, self.search)
        self.corr_feature = self.mask_model.mask.forward_corr(examplar_feature, self.search)
